...
// English defaults, used only if the server-side seed (and the DB) are empty.
...
export default function PrivacyTermsContent({
  ...
  // Seed first paint with the server-fetched translations (as fallbacks), so the
  // page renders the real content immediately instead of the English defaults
  // and then re-rendering once the client fetch resolves (no translation FOUC).
  const fallbacks = useMemo(() => ({ ...DEFAULTS, ...seeded }), [seeded]);
  const { t, updatedAt } = useTranslations("privacyTerms", fallbacks);
